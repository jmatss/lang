...
impl TyEnv {
    ...
    pub fn to_string_type_id(&self, ty_ctx: &TyCtx, type_id: TypeId) -> LangResult<String> {
        let mut result = String::new();

        let inf_type_id = ty_ctx.inferred_type(type_id)?;
        match self.ty(inf_type_id)? {
            Ty::CompoundType(inner_ty, gens, ..) => {
                result.push_str(&self.to_string_inner_ty(ty_ctx, inner_ty));
            ...
            Ty::Pointer(type_id_i, ..) => {
                result.push('{');
                result.push_str(&self.to_string_type_id(ty_ctx, *type_id_i)?);
                result.push('}');
            }
            ...
            Ty::Fn(gen_type_ids, params, ret_type_id_opt, ..) => {
                ...
                if !gen_type_ids.is_empty() {
                    result.push('<');
                    result.push_str(&self.to_string_list(ty_ctx, &gen_type_ids)?);
                    result.push('>');
                }

                result.push('(');
                if !params.is_empty() {
                    result.push_str(&self.to_string_list(ty_ctx, &params)?);
                }
                result.push(')');

                if let Some(ret_type_id) = ret_type_id_opt {
                    result.push_str("->");
                    result.push_str(&self.to_string_type_id(ty_ctx, *ret_type_id)?);
                }
            }
            ...
            Ty::UnknownArrayMember(arr_type_id, ..) => {
                result.push_str("arr(");
                result.push_str(&self.to_string_type_id(ty_ctx, *arr_type_id)?);
                result.push(')')
            }
        }
    ...
    pub fn to_string_inner_ty(&self, ty_ctx: &TyCtx, inner_ty: &InnerTy) -> String {
        match inner_ty {
            ...
            | InnerTy::Enum(path)
            | InnerTy::Union(path)
            | InnerTy::Trait(path) => self.to_string_path(ty_ctx, path),
            ...
            InnerTy::UnknownIdent(path, ..) => {
                format!("unknown_ident({})", self.to_string_path(ty_ctx, path))
            }
        }
    }

    pub fn to_string_generics(&self, ty_ctx: &TyCtx, generics: &Generics) -> LangResult<String> {
        let type_ids = generics.iter_types().copied().collect::<Vec<_>>();
        Ok(format!("<{}>", self.to_string_list(ty_ctx, &type_ids)?))
    }

    pub fn to_string_path(&self, ty_ctx: &TyCtx, path: &LangPath) -> String {
        const SEP: &str = "::";
        path.parts
            .iter()
            .map(|path_part| self.to_string_path_part(ty_ctx, path_part))
            .collect::<Vec<_>>()
            .join(SEP)
    }

    pub fn to_string_path_part(&self, ty_ctx: &TyCtx, path_part: &LangPathPart) -> String {
        if let Some(generics) = &path_part.1 {
            util::to_generic_name(ty_ctx, &path_part.0, generics)
        } else {
            path_part.0.clone()
        }
    }

    pub fn to_string_list(&self, ty_ctx: &TyCtx, ids: &[TypeId]) -> LangResult<String> {
        let mut result_vec = Vec::default();
        for type_id in ids {
            result_vec.push(self.to_string_type_id(ty_ctx, *type_id)?);
        }
