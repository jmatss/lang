...
use crate::parser::token::Expression::{Literal, Integer, Float};
use crate::parser::token::Literal::{CharLiteral, StringLiteral};
use crate::parser::token::{Function, Token};
use inkwell::builder::Builder;
use inkwell::context::Context;
use inkwell::module::Module;
use inkwell::passes::PassManager;
use inkwell::values::{AnyValue, FloatValue, FunctionValue, IntValue, PointerValue};
...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    ...
    fn compile_literal(
        &mut self,
        literal: &Literal,
    ) -> CustomResult<dyn AnyValue<'ctx>> {
        Ok(match literal {
            Literal(StringLiteral(s)) => {
                self.context.const_string(s.as_bytes(), false)
            },

            Literal(CharLiteral(c)) => {
                // Represents a character as a 4 byte u32.
                self.context.i32_type().const_int(c.parse()?, false)
            },

            _ => return Err(GenerationError(
                "Bad literal type, neither char nor string".to_string()
            ))
        })
    }
    ...
    ) -> Result<dyn AnyValue<'ctx>, &'static str> {
        match expr {
            Token::Expression(Literal(literal)) => {
                self.compile_literal(literal)
            },

            Token::Expression(Integer(integer)) => self.context.
        }
