...
impl<'a> TyCtx {
    ...
    fn solve_unknown_adt_member(
        ...
        if let Some(new_new_type_id) = self.replace_gen_impls(ast_ctx, new_type_id, &adt_gens)? {
    ...
    fn solve_unknown_adt_method(
        ...
        let inf_new_type_id = if let Some(mut new_type_id) = method.ret_type {
            if let Some(new_new_type_id) =
                self.replace_gen_impls(ast_ctx, new_type_id, &fn_call_gens)?
            {
                new_type_id = new_new_type_id;
            }
            if let Some(new_new_type_id) =
                self.replace_gen_impls(ast_ctx, new_type_id, &adt_gens)?
            {
                new_type_id = new_new_type_id;
            }
    ...
    fn solve_unknown_method_generic(
        ...
        if let Some(method_generics) = &self.new_method_generics(&method, &type_info)? {
            if let Some(new_new_type_id) =
                self.replace_gen_impls(ast_ctx, new_type_id, &method_generics)?
            {
                new_type_id = new_new_type_id;
            }
        }
    ...
    pub fn replace_gen_impls(
        ...
        let ty_was_updated = match &mut ty_clone {
            Ty::Generic(ident, ..) => {
                if let Some(impl_type_id) = generics_impl.get(ident) {
                    ty_clone = self.ty_env.ty(impl_type_id)?.clone();
                    true
                } else {
                    false
                }
            ...
            Ty::GenericInstance(ident, unique_id, ..) => {
                if let Some(impl_type_id) = generics_impl.get(ident) {
                    match self.ty_env.ty(impl_type_id)? {
                        Ty::Generic(..) => false,
            ...
            Ty::CompoundType(inner_ty, gens_clone, ..) => {
                ...
                for gen_type_id in gens_clone.iter_types_mut() {
                    if let Some(new_gen_type_id) =
                        self.replace_gen_impls(ast_ctx, *gen_type_id, generics_impl)?
                    {
            ...
            | Ty::UnknownArrayMember(type_id_i, ..) => {
                if let Some(new_type_id_i) =
                    self.replace_gen_impls(ast_ctx, *type_id_i, generics_impl)?
                {
            ...
            Ty::Fn(gens, params, ret_type_id_opt, ..) => {
                ...
                if let Some(ret_type_id) = ret_type_id_opt {
                    if let Some(new_ret_type_id) =
                        self.replace_gen_impls(ast_ctx, *ret_type_id, generics_impl)?
                    {
                ...
                for param_type_id in params.iter_mut() {
                    if let Some(new_param_type_id) =
                        self.replace_gen_impls(ast_ctx, *param_type_id, generics_impl)?
                    {
        ...
        Ok(if ty_was_updated {
            let new_type_id = self.ty_env.id(&ty_clone)?;
            self.solve(ast_ctx, new_type_id)?;
            Some(self.inferred_type(new_type_id)?)
        } else {
            None
        })
    }
